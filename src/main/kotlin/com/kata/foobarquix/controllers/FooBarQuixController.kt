...
class FooBarQuixController(private val  fooBarQuixService: FooBarQuixService) {
    ...
    @GetMapping( "/range/{inputNumber}", produces = [MediaType.APPLICATION_JSON_VALUE])
    @CrossOrigin(origins = ["http://localhost:8080", "http://localhost:4200" ])
    fun convertNumberToInt(@PathVariable(name = "inputNumber") inputNumber : Int) : ResultDto{
        return ResultDto(fooBarQuixService.convertNumberToInt(inputNumber))
    }
